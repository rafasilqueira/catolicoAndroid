...
class CatolicoMainActivity : AppCompatActivity(), OnNavigationItemSelectedListener {
    ...
    fun setupMenuItemDV() {
        if (usuario != null) {
            ...
            if (StatusFacebookLogin.isFacebookLoggedIn(this@CatolicoMainActivity)) {
                println(StatusFacebookLogin.isFacebookLoggedIn(this@CatolicoMainActivity))
                val profile = Profile.getCurrentProfile()
                Picasso.with(this@CatolicoMainActivity).load(profile?.getProfilePictureUri(200, 200).toString()).placeholder(R.drawable.ic_account_circle_white_96dp).error(R.drawable.ic_account_circle_white_96dp).into(imgUser)
            }
        } else {
